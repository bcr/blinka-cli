...
def do_update(args):
    ...
    (version, board_id) = blinka.board.identify(args.root)
    ...
    if args.firmware_version is None:
        ...
        url = blinka.board.get_download_url(new_version, board_id, 'uf2', args.locale)
    else:
    ...
    if args.commit_hash is not None:
        from blinka.s3util import find_firmware_by_hash
        s3_path = find_firmware_by_hash(args.commit_hash, board_id, args.locale)
        if s3_path:
            perform_update = True
            url = blinka.urlutil.get_s3_url(s3_path)
    ...
    if perform_update:
        ...
        if confirm(warning_string):
            ...
            time.sleep(4)
            # Get root to copy to
            bootloader_root = blinka.fsutil.find_circuit_python_bootloader_mode_root()
...
def setup_argument_parser(parser):
    ...
    root = blinka.fsutil.find_circuit_python_user_mode_root()
    ...
    parser.add_argument("--board", action="store", dest="board", help="specify the blinka.board type")
